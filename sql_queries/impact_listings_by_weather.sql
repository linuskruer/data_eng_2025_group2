-- Listings per day by weather condition across East Coast (joins on date and location)
WITH wx_bucket AS (
  SELECT
    w.location_key,
    w.date AS date_key,
...
SELECT
  d.date_key AS date,
  x.weather_bucket,
  COUNT(f.listing_key) AS listings
FROM fact_listings f
JOIN dim_date d ON f.date_key = d.date_key
JOIN wx_bucket x ON f.location_key = x.location_key AND f.date_key = x.date_key
GROUP BY d.date_key, x.weather_bucket
ORDER BY d.date_key, x.weather_bucket;
